...
if (is.null(opt$ratios) || is.null(opt$subset_file) || is.null(opt$subset) || is.null(opt$outdir) || !is.null(opt$help) || is.null(opt$cores)) {
...
eg.outfile = paste(opt$outdir, paste('biclusterEigengenes_',opt$subset,'.csv',sep=''), sep='/')
eg.ve.outfile = paste(opt$outdir, paste('biclusterVarianceExplained_',opt$subset,'.csv',sep=''), sep='/')
#source(opt$subset_file)
subsets = fromJSON(file=opt$subset_file)
print(opt$subset)
